...
def load_model_and_tokenizer(config):
    ...
    if config.num_labels == 1:
        model = AutoModelForSequenceClassification.from_pretrained(config.model_name, num_labels=config.num_labels, ignore_mismatched_sizes=True)
    else:
...
def make_trainer(train_config, model, tokenizer):
    ...
    # 데이터
    train_dataset, valid_dataset = get_dataset(train_config, tokenizer=tokenizer, type_='train', submission=False), get_dataset(train_config, tokenizer=tokenizer, type_='valid', submission=False)
    ...
    lr_scheduler = get_linear_schedule_with_warmup(optimizer=optimizer,
                                                   ...
                                                   num_training_steps=((len(train_dataset)-1) // training_args.train_batch_size + 1) * training_args.num_train_epochs)

    early_stopping = EarlyStoppingCallback(train_config.patience, train_config.threshold)
    
    loss_name = 'CrossEntropy' if (train_config.model_name == 'beomi/korean-hatespeech-multilabel') and (train_config.num_labels==2) else None
...
def inference(config):
    ...
    test_dataset = get_dataset(config, tokenizer=tokenizer, type_='test', submission=False)
    submission_df = get_dataset(config, tokenizer=tokenizer, type_='test', submission=True)
